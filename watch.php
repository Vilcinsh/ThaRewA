...
<link rel="stylesheet" href="https://rew.vissnavslikti.lv/assets/css/watch.css" type="text/css">
    ...
    <div class="video-section">
      <div class="video-wrapper">
        <div id="videoContainer">
          ...
          <!-- Fullscreen episode toggle (appears in fullscreen) -->
        ...
        <div class="video-info">
          ...
          <p class="episode-title" id="episodeTitle">Episode <?php echo htmlspecialchars($episodeNumber); ?></p>
        </div>
        ...
        <div class="nav-controls">
          <button class="nav-btn" id="prevBtn" onclick="changeEpisode(-1)"><i class="fas fa-chevron-left"></i> Previous</button>
          <button class="nav-btn" id="nextBtn" onclick="changeEpisode(1)">Next <i class="fas fa-chevron-right"></i></button>
        </div>
      </div>
    </div>
...
const KODIK_API_BASE = 'https://kodik.rewcrew.lv'; // NEW: Your unified Kodik API
const PROXY_BASE = 'https://corsproxy.rewcrew.lv/proxy';
const ANISKIP_API = 'https://api.aniskip.com/v2/skip-times';

const animeId = '<?php echo htmlspecialchars($animeId); ?>';
let currentEpisode = <?php echo (int)$episodeNumber; ?>;
...
const preferences = {
  ...
  preferredLanguage: '<?= htmlspecialchars($userSettings['preferred_language'] ?? 'auto') ?>'
};
...
/* =========================================================
   NETWORK LAYER (FAST + SAFE)
   - JSON requests: direct (NO proxy)
   - Media requests: via proxy
   - sessionStorage cache + in-flight dedupe
========================================================= */
...
function buildFsSubtitlesOverlay() {
  ...
  if (!isIframeMode && activeSubTracks.length > 0) {
    activeSubTracks.forEach((track, idx) => {
      ...
      btn.onclick = () => {
        applySubtitleSelection(String(idx));
        buildFsSubtitlesOverlay();
      };
      grid.appendChild(btn);
    });
...
function setupVideoEvents() {
  ...
  video.addEventListener('timeupdate', () => {
    ...
    if (videoData.intro && preferences.autoSkipIntro) {
      if (video.currentTime >= videoData.intro.start && video.currentTime < videoData.intro.start + 1) {
        video.currentTime = videoData.intro.end;
      }
    }
    if (videoData.outro && preferences.autoSkipOutro) {
      if (video.currentTime >= videoData.outro.start && video.currentTime < videoData.outro.start + 1) {
        video.currentTime = videoData.outro.end;
      }
    }

    if (videoData.outro && preferences.autoplay && currentEpisode < (animeData?.episodes?.length || 0) &&
        !nextEpisodeTimer && !nextEpisodeCanceled && video.currentTime >= videoData.outro.start) {
...
async function loadAnimeInfo() {
  try {
    ...
    mergedData = await fetchJSON(`${MERGE_API}?slug=${encodeURIComponent(animeId)}`, {
    ...
    if (mergedData.shikimori_id) {
      await loadKodikData(mergedData.shikimori_id);
    }
...
/* =========================================================
   NEW KODIK API FUNCTIONS
========================================================= */
async function loadKodikData(shikimoriId) {
  try {
    const data = await fetchJSON(`${KODIK_API_BASE}/kodik/search-by-id?anime_id=${shikimoriId}&id_type=shikimori`, {
      cacheKey: `kodik:${shikimoriId}`,
      ttlMs: 10 * 60_000
    });
    
    if (data?.success && data.results?.length > 0) {
      kodikData = data.results;
      console.log('✓ Kodik data loaded:', kodikData.length, 'results');
    }
  } catch(err) {
    console.error('Kodik load failed:', err);
  }
}

async function getKodikInfo(animeId) {
  try {
    const data = await fetchJSON(`${KODIK_API_BASE}/kodik/info?anime_id=${animeId}&id_type=shikimori`, {
      cacheKey: `kodik_info:${animeId}`,
      ttlMs: 10 * 60_000
    });
    ...
    console.error('Kodik info failed:', err);
    return null;
  }
}
...
async function getKodikStreamUrl(animeId, episode, translationId, quality = 720) {
  try {
    const data = await fetchJSON(
      ...
      {
        cacheKey: `kodik_stream:${animeId}:${episode}:${translationId}:${quality}`,
        ttlMs: 5 * 60_000
      }
    );
    ...
    console.error('Kodik stream URL failed:', err);
    throw err;
  }
}
...
function renderAnimeInfo() {
  ...
  const d = document.getElementById('animeDetails');
  d.innerHTML = `
...
function renderEpisodes() {
  ...
  animeData.episodes.forEach(ep => {
    ...
    if (ep.number === currentEpisode) a.classList.add('active');
...
async function loadEpisode(num) {
  ...
  const isKodikActive = videoData?.isKodik;

  if (isKodikActive && kodikData?.length) {
...
async function loadServers(episodeId) {
  ...
  try {
    const data = await fetchJSON(`${API_BASE}/episode/servers?animeEpisodeId=${encodeURIComponent(episodeId)}`, {
    ...
    console.warn('English servers failed', err);
  }
  ...
  if (kodikData?.length > 0) {
    ...
    if (userWatchPref?.provider === 'kodik') {
      ...
      if (translationId) {
        await loadKodikStream(mergedData.shikimori_id, currentEpisode, translationId);
      }
...
function createServerButton(displayName, badgeText, badgeClass = '', onClick) {
  const btn = document.createElement('button');
...
function renderEnglishServers(servers, episodeId) {
  ...
  const categories = [
    {key: 'sub', label: 'SUB', color: 'sub'},
    {key: 'dub', label: 'DUB', color: 'dub'}
  ];

  categories.forEach(cat => {
    const serverList = (servers[cat.key] || []).filter(s =>
      !SERVER_BLACKLIST.some(bad => s.serverName.toLowerCase().includes(bad))
    );

    serverList.forEach(s => {
      const displayName = DISPLAY_NAMES[s.serverName.toLowerCase()] || s.serverName;
      const btn = createServerButton(displayName, cat.label, cat.color, async () => {
        const pref = { provider: 'hianime', category: cat.key, server: s.serverName };
        userWatchPref = pref; savePrefLocal(pref); savePrefServer(pref);
        loadVideo(episodeId, s.serverName, cat.key);
      });
      c.appendChild(btn);
    });
  });
...
async function renderRussianServers() {
  ...
  // Get detailed info for better quality display
  ...
  // Use info from new API if available
  ...
  if (translations.length > 0) {
    translations.forEach((trans) => {
      ...
      const displayName = trans.title || trans.name || 'Unknown';

      const btn = createServerButton(displayName, '', badgeClass, async () => {
        const pref = {
          provider: 'kodik',
          category: isVoice ? 'voice' : 'rus_sub',
          server: displayName,
          kodik_translation_id: trans.id
        };
        userWatchPref = pref; 
        savePrefLocal(pref); 
        savePrefServer(pref);
  ...
  } else if (kodikData?.length > 0) {
    // Fallback to search results if info endpoint fails
    kodikData.forEach((result) => {
      const trans = result.translation;
      if (!trans) return;
      
      const isVoice = trans.type === 'voice';
      const badgeClass = isVoice ? 'dub' : 'sub';
      const displayName = trans.title || 'Unknown';
...
async function tryAutoSelectServer(servers, episodeId) {
  ...
  if (userWatchPref?.category && categories.includes(userWatchPref.category)) {
    categories.sort((a, b) => (a === userWatchPref.category ? -1 : 1));
  }
...
/* =========================================================
   HIANIME PLAYER (NO JSON PROXY) + 403 fallback
========================================================= */
...
async function loadVideo(episodeId, serverName, category) {
  ...
  try {
    ...
    const url = `${API_BASE}/episode/sources?animeEpisodeId=${encodeURIComponent(episodeId)}&server=${encodeURIComponent(serverName)}&category=${category}`;
    const data = await fetchJSON(url, {
      cacheKey: `sources:${episodeId}:${serverName}:${category}`,
      ttlMs: 60_000
    });
    ...
    document.getElementById('qualityBadge') && (document.getElementById('qualityBadge').textContent = category.toUpperCase());

    document.querySelectorAll('.server-btn').forEach(b => {
      const displayName = DISPLAY_NAMES[serverName.toLowerCase()] || serverName;
      if (b.textContent.includes(displayName) && b.classList.contains(category)) b.classList.add('active');
    });
    ...
    if (category === 'dub' && isForbiddenError(err) && lastServersSnapshot) {
      showError('DUB blocked on this server — trying fallback…');
...
/* =========================================================
   KODIK STREAM PLAYBACK (NEW API - DIRECT HLS)
========================================================= */
async function loadKodikStream(shikimoriId, episode, translationId, quality = 720) {
  ...
  try {
    ...
    // Get skip times
    let skipTimes = { intro: null, outro: null };
    if (shikimoriId) {
      skipTimes = await fetchSkipTimes(shikimoriId, episode);
    }

    // Get HLS stream URL from new API
    const streamUrl = await getKodikStreamUrl(shikimoriId, episode, translationId, quality);

    videoData = { 
      sources: [{ url: streamUrl, quality: quality }], 
      tracks: [], 
    ...
    await ensureVideoElement();
    await playKodikHLS(streamUrl, quality);

    // Mark active server button
    document.querySelectorAll('.server-btn').forEach(b => {
      if (b.onclick?.toString().includes(translationId)) {
...
async function playKodikHLS(streamUrl, quality = 720) {
  showLoading(true);
  try {
      ...
      window.hls.on(Hls.Events.ERROR, function(event, data) {
        if (data.fatal) {
          if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
            setTimeout(() => window.hls?.startLoad(), 800);
          } else if (data.type === Hls.ErrorTypes.MEDIA_ERROR) {
            window.hls.recoverMediaError();
          } else {
            showError('Playback error - try another server');
            window.hls.destroy();
          }
        }
      });
    ...
    document.getElementById('qualityBadge') && (document.getElementById('qualityBadge').textContent = `RUS • ${quality}p`);

  } catch (err) {
    console.error('[KODIK HLS] Error:', err);
    showError('Failed to play Kodik stream');
  } finally {
    showLoading(false);
  }
}
...
async function ensureVideoElement() {
  return new Promise((resolve, reject) => {
    ...
    const checkElement = () => {
      const v = document.getElementById('videoPlayer');
      if (v) { video = v; resolve(v); return; }
      attempts++;
      if (attempts >= maxAttempts) { reject(new Error('Video element not found after 5 seconds')); return; }
...
/* =========================================================
   PLAY VIDEO (proxy only media)
========================================================= */
async function playVideo(masterUrl) {
  const v = document.getElementById('videoPlayer');
  if (!v) throw new Error('Video element not found');

  const proxied = proxify(masterUrl);
...
function addSubtitles(tracks) {
  ...
  tracks.forEach(track => {
    ...
    const url = proxify(track.url);
    fetch(url).then(r => r.text()).then(vtt => {
      parseVTT(vtt).forEach(cue => tt.addCue(new VTTCue(cue.start, cue.end, cue.text)));
      buildFsSubtitlesOverlay();
    }).catch(() => {});
  });
}
...
async function rebuildVideoPlayer() {
  ...
  container.innerHTML = `
    ...
    <div class="fs-episodes" id="fsEpisodes" aria-hidden="true">
      <div class="fs-episodes-header"><span>Episodes</span><button type="button" class="fs-episodes-close" id="fsEpisodesClose" aria-label="Close">&times;</button></div>
      <div class="fs-episodes-grid" id="fsEpisodesGrid"></div>
    </div>

    <div class="fs-episodes" id="fsSubtitles" aria-hidden="true" style="max-width: 300px;">
      <div class="fs-episodes-header"><span>Subtitles</span><button type="button" class="fs-episodes-close" id="fsSubtitlesClose" aria-label="Close">&times;</button></div>
...
function updateNavButtons() {
  ...
  if (nextBtn) nextBtn.disabled = currentEpisode >= animeData.episodes.length;
}
...
function saveProgress(isCompleted = false, useBeacon = false) {
  ...
  if (useBeacon && navigator.sendBeacon) {
    ...
    navigator.sendBeacon('/api/watch/progress/save.php', blob);
    return;
  }
